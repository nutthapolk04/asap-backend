  ...
  <div class="check-in-detail" v-loading="loading">
    ...
    <div class="detail-content" v-if="checkIn.id">
      ...
      <div class="content-card documents-section">
        <div class="section-title">
          ...
          เอกสารที่แนบมา
          <span class="doc-count">({{ checkIn.documents?.length || 0 }} ไฟล์)</span>
        </div>
        <div class="documents-notice">
          <el-icon><InfoFilled /></el-icon>
          กรุณาตรวจสอบเอกสารทั้งหมดก่อนอนุมัติหรือยกเลิก
        </div>

        <div class="documents-grid" v-if="checkIn.documents?.length">
          <div
            class="document-card"
            v-for="(doc, index) in checkIn.documents"
            :key="index"
          >
        ...
        <el-empty v-else description="ไม่มีเอกสารที่แนบมา" />
      </div>
      ...
      <div class="content-card" v-if="checkIn.status === 'rejected' && checkIn.rejectReason">
        ...
        <p class="action-info">
          ยกเลิกโดย: {{ checkIn.actionBy }} | {{ formatDateTime(checkIn.actionDate) }}
        </p>
      </div>
      ...
      <div class="action-section" v-if="checkIn.status === 'pending'">
        <div class="action-card">
          <div class="action-notice">
            <el-icon><InfoFilled /></el-icon>
            <span>กรุณาตรวจสอบข้อมูลและเอกสารให้ครบถ้วนก่อนดำเนินการ</span>
          </div>
    ...
    >
      <el-form :model="rejectForm" label-position="top">
        <el-form-item label="เหตุผลที่ยกเลิก" required>
          <el-input
            v-model="rejectForm.reason"
            type="textarea"
            :rows="4"
            placeholder="กรุณาระบุเหตุผลที่ยกเลิกการเช็คอิน..."
          />
        </el-form-item>
      </el-form>
      <template #footer>
